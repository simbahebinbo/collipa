...
class BaseForm(object):
    ...
    @property
    def data(self):
        return dict((name, f.data) for name, f in list(self._fields.items()))

    @property
    def errors(self):
        if self._errors is None:
            self._errors = dict((name, f.errors) for name, f in list(self._fields.items()) if f.errors)
        return self._errors
